...
# key: chrom, value: another dictionary w/ key: position, value: tuple that is list of sequences and qualities
...
def populate_dict(
    ...
    with get_ih(path) as IH:
        for line in tqdm(IH, "Record"): #tqdm is progress bar
            chrom, pos, seq, qual = line.strip().split("\t")
            pos = int(pos) - cs_len
            umi_dict[chrom][pos][0].append(seq) #adds all sequences at that position
            umi_dict[chrom][pos][1].append(qual) #adds all qualities at that position
    return umi_dict
...
with get_oh(args.output, args.compress_level) as OH:
    ...
    for chrom, pos_dict in tqdm(umi_dict.items(), desc="Chromosome"): #the desc makes sure its sorted
